...
export async function POST(req: NextRequest) {
  ...
  const policyInstruction =
    "Use the supplied question context and any candidate examples to answer naturally. If the user asks for a similar question, harder question, quiz, year, source, or whether it is a JAMB question, handle it in a grounded way from the provided context. Default behavior: always return a real JAMB question from provided context when one exists. Never fabricate and label as JAMB. Every returned question must include a source line. If JAMB: write 'Source: JAMB (Year: YYYY)' using the actual year from context when available, or 'Source: JAMB (Year: Unknown)' if missing. If not JAMB: write 'Source: Practice (Not from JAMB database)'. For any request asking for JAMB, return only from provided context candidates; if no suitable context item exists, say that clearly and ask whether to provide a clearly labeled practice question. When rendering any math, use LaTeX delimiters such as \\\\( ... \\\\) or \\\\[ ... \\\\]. Keep quiz responses interactive and do not explain until the user answers unless the user explicitly asks for an explanation.";
